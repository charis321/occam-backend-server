...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
	...
	public List<User> findUsersByEmail(List<String> emails) {
		...
		Set<String> emailSet = emails.stream().filter(StringUtils::hasText).collect(Collectors.toSet());

		if (emailSet.isEmpty())
			return Collections.emptyList();

		return this.lambdaQuery().in(User::getEmail, emailSet).list();
	}
	...
	public void uploadAvatar(Long userId, MultipartFile file) {
		if(userId==null) throw new BusinessException(400, "用戶id不可為空");
		if(ObjectUtils.isEmpty(file)) throw new BusinessException(400, "上傳檔案不可為空");
		
		log.info("file",file.toString());
		String avatar = fileUploadService.uploadAvatar(userId, file);
		this.lambdaUpdate().eq(User::getId, userId).set(User::getAvatar, avatar).update();
